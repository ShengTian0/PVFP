# -*- coding: utf-8 -*-
"""PVFP 与 Algorithm2 统一成本配置的对比实验脚本 (constrast1_1)

实验设置：
- 使用 small 拓扑和同一批 SFC 请求；
- 将所有链路成本统一为 5；
- 将所有功能节点的部署成本预算上限统一为 9999；
- 分别在所有 VNF 部署成本统一为 {1, 3, 5, 7, 9} 时，比较 PVFP 与 Algorithm2 的总成本；
...
UNIFORM_LINK_COST = 5
UNIFORM_BUDGET_CAP = 20
DEPLOY_COST_VALUES = [5]
...
def run_uniform_cost_experiments(num_sfcs: int = 30):
    """在统一成本设置下，针对多组部署成本值比较 PVFP 与 Algorithm2。"""
    if VERBOSE:
        print("\n" + "=" * 80)
        print(" " * 15 + "PVFP 与 Algorithm2 统一成本配置对比实验 (constrast1_1)")
        print("=" * 80)
...
if __name__ == "__main__":
    VERBOSE = True
    run_uniform_cost_experiments(num_sfcs=30)
