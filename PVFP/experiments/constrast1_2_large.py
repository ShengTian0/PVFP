# -*- coding: utf-8 -*-
"""PVFP 与 Algorithm2 统一链路成本配置的大规模网络对比实验脚本 (constrast1_2_large)

实验设置：
- 使用 large 拓扑和同一批 SFC 请求；
- 所有功能节点上所有 VNF 的部署成本统一为 5；
- 所有功能节点的部署成本预算上限统一为 9999；
- 链路成本依次取 {6, 13, 15, 17, 19}，比较 PVFP 与 Algorithm2 的总成本；
...
UNIFORM_DEPLOY_COST = 5.0
UNIFORM_BUDGET_CAP = 9999.0
LINK_COST_VALUES = [6.0, 13.0, 15.0, 17.0, 19.0]
...
def print_uniform_cost_snapshot(pvfp: PVFPFramework, link_cost: float) -> None:
...
def run_uniform_link_experiments_large(num_sfcs: int = 30):
    """在统一部署成本=5、预算=9999 下，对不同链路成本在 large 拓扑上比较 PVFP 与 Algorithm2。"""
    if VERBOSE:
        print("\n" + "=" * 80)
        print(" " * 15 + "PVFP 与 Algorithm2 统一链路成本大规模网络对比实验 (constrast1_2_large)")
        print("=" * 80)
    ...
    # 2. 生成一批固定的 SFC 请求，供所有链路成本设置共用
    test_sfcs = pvfp.sfc_generator.generate_batch_sfcs(num_sfcs)

    summary_results = []  # (link_cost, num_common, avg_pvfp, avg_algo2)

    for link_cost in LINK_COST_VALUES:
        if VERBOSE:
            print("\n" + "-" * 80)
            print(f"链路成本统一为 {link_cost:.1f} 时的对比结果：")
            print("-" * 80)
        ...
        # 2.1 在 PVFP 拓扑上施加统一成本设置（部署成本始终为 UNIFORM_DEPLOY_COST）
        apply_uniform_costs(
            pvfp,
            link_cost=link_cost,
            budget_cap=UNIFORM_BUDGET_CAP,
            deploy_cost_val=UNIFORM_DEPLOY_COST,
        )

        # 打印当前统一成本配置的快照，便于验证设置是否生效
        print_uniform_cost_snapshot(pvfp, link_cost)
        ...
        if not common_indices:
            if VERBOSE:
                print("\n[提示] 在本次链路成本设置下，两种方法没有在同一条 SFC 上同时成功，无法计算平均成本对比。")
            summary_results.append((link_cost, 0, 0.0, 0.0))
            continue
        ...
        if VERBOSE:
            print("\n" + "=" * 80)
            print(f"链路成本={link_cost:.1f} 时，共 {n} 条 SFC 两种方法均成功部署：")
            print(f"  PVFP 平均成本:      {avg_pvfp:.2f}")
            print(f"  Algorithm2 平均成本: {avg_algo2:.2f}")
            print("=" * 80)

        summary_results.append((link_cost, n, avg_pvfp, avg_algo2))

    # 3. 汇总各个链路成本配置下的结果
    if VERBOSE:
        print("\n" + "#" * 80)
        print("各统一链路成本配置下的大规模网络总体结果汇总：")
        print("(链路成本, 共同成功SFC数, PVFP平均成本, Algorithm2平均成本)")
        for link_cost, n, avg_pvfp, avg_algo2 in summary_results:
            print(
                f"  ce={link_cost:.1f}: N_common={n}, "
...
if __name__ == "__main__":
    run_uniform_link_experiments_large(num_sfcs=30)
