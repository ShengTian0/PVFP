# -*- coding: utf-8 -*-
"""PVFP 与 Algorithm2 统一链路成本配置的对比实验脚本 (constrast1_2)

实验设置：
- 使用 small 拓扑和同一批 SFC 请求；
- 所有功能节点上所有 VNF 的部署成本统一为 5；
- 所有功能节点的部署成本预算上限统一为 9999；
- 链路成本依次取 {1, 3, 5, 7, 9}，比较 PVFP 与 Algorithm2 的总成本；
...
def run_uniform_link_experiments(num_sfcs: int = 30):
    """在统一部署成本=5、预算=9999 下，对不同链路成本比较 PVFP 与 Algorithm2。"""
    if VERBOSE:
        print("\n" + "=" * 80)
        print(" " * 15 + "PVFP 与 Algorithm2 统一链路成本对比实验 (constrast1_2)")
        print("=" * 80)
...
if __name__ == "__main__":
    run_uniform_link_experiments(num_sfcs=30)
